...
impl Parser {
    ...
    fn consume_identifier(&mut self, message: &str) -> SusumuResult<&str> {
        if self.check(&TokenType::Identifier) {
...
mod tests {
    ...
    #[test]
    fn test_module_arrow_chain() {
        // Test that import/export are treated as regular arrow chains
        let source = "payment_module -> from <- import <- (processPayment, refundPayment)";
        let mut lexer = Lexer::new(source);
        let tokens = lexer.tokenize().unwrap();
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();

        // Should parse as regular arrow chain expression
        assert!(ast.main_expression.is_some());

        // Verify it parsed as an arrow chain with the expected structure
        if let Some(Expression::ArrowChain {
            expressions,
            directions,
        }) = &ast.main_expression
        {
            assert_eq!(expressions.len(), 4); // payment_module, from, import, (processPayment, refundPayment)
            assert_eq!(directions.len(), 3); // ->, <-, <-
            assert_eq!(directions[0], ArrowDirection::Forward); // ->
            assert_eq!(directions[1], ArrowDirection::Backward); // <-
            assert_eq!(directions[2], ArrowDirection::Backward); // <-
        } else {
            panic!("Expected arrow chain for module import statement");
        }
    }

    #[test]
    fn test_export_arrow_chain() {
        // Test export as arrow chain: (func1, func2) -> export
        let source = "(processPayment, refundPayment) -> export";
        let mut lexer = Lexer::new(source);
        let tokens = lexer.tokenize().unwrap();
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
        ...
        {
            assert_eq!(expressions.len(), 2); // (func1, func2), export
            assert_eq!(directions.len(), 1); // ->
            assert_eq!(directions[0], ArrowDirection::Forward);
        } else {
            panic!("Expected arrow chain for export statement");
        }
    }
}
