...
use crate::interpreter::{ExecutionTrace, ExecutionStepType};
...
impl VisualDebugger {
    ...
    pub fn set_breakpoint(&mut self, line: usize, column: Option<usize>, condition: Option<String>) -> usize {
    ...
    pub fn generate_execution_report(&self) -> String {
        ...
        for (i, path) in self.flow_paths.iter().enumerate() {
            report.push_str(&format!("Flow Path {} (line {}):\n", i + 1, path.start_line));
            ...
            for (j, step) in path.steps.iter().enumerate() {
                report.push_str(&format!("    Step {}: {} -> {}\n", 
                    j + 1, step.input_type.description(), step.output_type.description()));
            }
            report.push('\n');
        }
        ...
        for (i, trace) in self.traces.iter().enumerate() {
            report.push_str(&format!("{}. {} (line {})\n", 
                i + 1, trace.expression, trace.line));
            ...
            match &trace.step_type {
                ...
                ExecutionStepType::ArrowBackward { from, to } => {
                    report.push_str(&format!("   {} <- {}\n", to, from));
                    report.push_str(&format!("   Convergent: {}\n", self.value_to_display(&trace.input_value)));
                }
                ...
                ExecutionStepType::Conditional { branch, condition_result } => {
                    report.push_str(&format!("   Branch: {} (condition: {})\n", 
...
<body>
    ...
    fn add_performance_analysis(&self, report: &mut String) {
        ...
        for (i, trace) in sorted_traces.iter().take(5).enumerate() {
            report.push_str(&format!("    {}. {} ({}ns)\n", 
                i + 1, trace.expression, trace.execution_time_ns));
        }
        report.push('\n');
    }
    ...
    fn add_debugging_recommendations(&self, report: &mut String) {
        ...
        let arrow_chains = self.traces.iter()
            .filter(|t| matches!(t.step_type, ExecutionStepType::ArrowForward { .. }))
            .count();
        
        let function_calls = self.traces.iter()
        ...
        report.push_str("  💡 Use 'i success {} e {}' patterns for better error flow visualization.\n");
        report.push_str("  💡 Set breakpoints at arrow junctions to inspect intermediate values.\n");
...
impl FlowDiagramGenerator {
    ...
    pub fn generate_ascii_diagram(&self, path: &ArrowFlowPath) -> String {
        ...
        diagram.push_str("┐\n");
        diagram.push_str(&format!("│ Arrow Flow Diagram (line {:<29} │\n", path.start_line));
        diagram.push_str("├");
        diagram.push_str(&"─".repeat(50));
        diagram.push_str("┤\n");
        ...
        if self.config.show_types {
            ...
            for (i, step) in path.steps.iter().enumerate() {
                let type_line = format!("  {}: {} → {}", 
                    i + 1, step.input_type.description(), step.output_type.description());
